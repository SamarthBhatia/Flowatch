...
namespace Firewall{
    ...
    bool ConnectionMonitor::start(){
        ...
        if (dev == nullptr){
            Logger::get() -> error("Failed to find default device: {}",errbuf);
        }

        handle_ = pcap_open_live(dev,BUFSIZ,1,100,errbuf);
        if (handle_ == nullptr){
            Logger::get()->error("Failed to open device {}: {}",dev,errbuf);
            return false;
        }
    ...
    void ConnectionMonitor::processPacket(const struct pcap_pkthdr* pkthdr, const u_char* packet){
        ...
        inet_ntop(AF_INET,&(ip->ip_dst),dstIP,INET6_ADDRSTRLEN);
        ...
        if (ip->ip_p == IPPROTO_TCP){
            ...
            Logger::get()->debug("TCP Connection: {}:{} -> {}:{}",srcIP,srcPort,dstIP,dstPort);
